...
def get_data(args, data_config):
    ...
    ic(timestamps.max() / (3600 * 24) + 1)
    n_days = round(float(timestamps.max() / (3600 * 24) + 1))
    n_samples = y.shape[0]
    ...
    for day in range(n_days):
        ...
        # get the inds of trans for each day (which trans belong to which day)
    ...
    # list of number of trans per day
...
def get_il_data(data_config):
    ...
    y_old = torch.LongTensor(df_old['Is Laundering'].to_numpy())
    timestamps_old = torch.tensor(df_old['Timestamp'].to_numpy(), dtype=torch.float)
    
    num_nodes_old = df_old[['from_id', 'to_id']].to_numpy().max() + 1
    x_old = torch.ones((num_nodes_old, 1), dtype=torch.float)  # Placeholder feature
    
    data_old = GraphData(
        x=x_old,
        y=y_old,
        edge_index=edge_index_old,
        edge_attr=edge_attr_old,
        timestamps=timestamps_old
    )
    ...
    ic(f"Old snapshot: {data_old.num_nodes} nodes, {data_old.num_edges} edges")
    ic(f"New snapshot: {data_new.num_nodes} nodes, {data_new.num_edges} edges")
